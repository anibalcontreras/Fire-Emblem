...
public static class ConditionalEffectBuilder
{
    public static MultiEffect BuildNeutralizationPenaltyEffect(ICondition condition, params StatType[] statTypes)
    {
        var effects = statTypes.Select(statType => 
            new ConditionalEffect(condition, EffectBuilder.BuildNeutralizationPenaltyEffect(statType, EffectTarget.Unit))
        ).ToArray();
    ...
    {
        var effects = statTypes.Select(statType => 
            new ConditionalEffect(condition, EffectBuilder.BuildPenaltyEffect(statType, penaltyValue, EffectTarget.Rival))
        ).ToArray();
    ...
    {
        var effects = statTypes.Select(statType => 
            new ConditionalEffect(condition, EffectBuilder.BuildBonusEffect(statType, bonusValue, EffectTarget.Unit))
        ).ToArray();
    ...
    {
        var effects = statTypes.Select(statType => 
            new ConditionalEffect(condition, EffectBuilder.BuildNeutralizationBonusEffect(statType, EffectTarget.Rival))
        ).ToArray();
